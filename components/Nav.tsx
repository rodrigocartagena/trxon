...
export default function Nav() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-bg-deep/80 backdrop-blur-md border-b-[0.5px] border-border py-[22px] px-5 md:px-[36px]">
        ...
        <nav className="hidden md:flex gap-8">
          {links.map((link) => (
            <Link
              key={link.path}
              href={link.path}
              className={`text-[13px] font-normal transition-colors duration-200 ease-out focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-bg-deep ${
      ...
      {/* Mobile Nav Dropdown */}
      {isOpen && (
        <nav className="md:hidden absolute top-full left-0 w-full bg-bg-deep border-b-[0.5px] border-border py-4 px-5 flex flex-col gap-4">
          {links.map((link) => (
            <Link
              key={link.path}
              href={link.path}
              onClick={() => setIsOpen(false)}
              className={`text-[15px] font-normal transition-colors duration-200 ease-out py-2 ${
                pathname === link.path ? "text-text-primary" : "text-text-body hover:text-text-primary"
              }`}
            >
              {link.name}
